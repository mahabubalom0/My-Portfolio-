...
const noteBlock = `
      <div class="project-card reveal" onclick="openGallery('writing_note', 0)">
        <div class="project-video-preview" style="width: 100%; height: 200px; overflow: hidden; position: relative; border-radius: 8px 8px 0 0;">
          <video src="/images/projects/writing_note/screen-20260809-224350.mp4" autoplay loop muted playsinline style="width: 100%; height: 100%; object-fit: cover;"></video>
          <div style="position: absolute; bottom: 10px; right: 10px; background: rgba(0,0,0,0.6); backdrop-filter: blur(4px); color: white; padding: 4px 10px; border-radius: 20px; font-size: 0.8rem; font-weight: 500; display: flex; align-items: center; gap: 6px; box-shadow: 0 4px 10px rgba(0,0,0,0.3);">
            <i class="fas fa-images"></i> <span id="writing-note-count">0 Images</span>
          </div>
        </div>
        <div class="project-body">
          <div class="project-title">Writing Note</div>
          <div class="project-desc">A beautiful note-taking app with a clean UI and smooth interactions.</div>
...
html = html.replace(/<div class="project-card reveal">\s*<div class="project-thumb"/, noteBlock);

fs.writeFileSync('index.html', html);
console.log('Added writing note project card.');
